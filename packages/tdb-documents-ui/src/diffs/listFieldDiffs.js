...
function displaySubDocumentElements(diffPatch, item, formData, startFormDataIndex, schema, label, required, interest, css, fullFrame, frame, type, choicesEqualList) {
    ...
    for(var fds=0; fds<formData[item].length; fds++) {
        ...
        if(Array.isArray(choicesEqualList)) choiceCss=choicesEqualList[fds] 
        for(var subDocKey in formData[item][fds]) {
            if(subDocKey==="@id") continue
                ...
                let rest=doOperation(diffPatch[fds][subDocKey], item, formData, startFormDataIndex, elementSchema.properties[subDocKey], elementSchema.properties[subDocKey].title, required, interest, css, fullFrame, frame, type, choicesEqualList)
                fields.push(rest)
                hasChanged=true
            }
            else {
