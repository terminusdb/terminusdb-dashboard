import {
    getTitle,
    addSelectedLanguageToDocumentation, 
    extractChoiceDocumentLabels, 
    addCustomUI, 
    checkIfKey, 
    isFilled, 
...
function getDocumentLayout(documentClass, fullFrame, current, item, uiFrame, mode, formData, onTraverse, onSelect, language) {
    var layout = {}
    ...
    let extractedDocumentation=addSelectedLanguageToDocumentation(documentation, language)

    let exractedProperties = getProperties (fullFrame, current, frame, uiFrame, mode, filledData, onTraverse, onSelect, extractedDocumentation)
    
    //console.log("exractedProperties", documentation, documentClass, exractedProperties, frame)
    // add subdocument type as @type field
    let docLabel=extractChoiceDocumentLabels(fullFrame, documentClass, language)
    let label=docLabel["@label"]
    //console.log("docLabel", docLabel, fullFrame, documentClass)
...
// Create Layout
export function getCreateLayout(fullFrame, current, frame, item, uiFrame, mode, formData, onTraverse, onSelect, documentation, language) {
    ...
    frame[item].map(fr => { 
        ...
        anyOfArray.push(getDocumentLayout(documentName, fullFrame, current, item, uiFrame, mode, formData, onTraverse, onSelect, language))
    })
...
// Edit Layout
export function getEditLayout(fullFrame, current, frame, item, uiFrame, mode, formData, onTraverse, onSelect, documentation, language) {
    // get choice documents
    let anyOfArray = []
    frame[item].map(fr => {
        var documentName
        ...
        let documentLayout= getDocumentLayout(documentName, fullFrame, current, item, uiFrame, mode, formData, onTraverse, onSelect, language)
...
// View Layout
export function getViewLayout(fullFrame, current, frame, item, uiFrame, mode, formData, onTraverse, onSelect, documentation, language) {
    // get choice documents
    let anyOfArray = []
    frame[item].map(fr => {
        var documentName
