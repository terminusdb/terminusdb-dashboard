...
const Form = ({frames}) => {
    ...
    const Search = ({setSelected}) => {
        ...
        function handleClick(e){
            if(setSelected) setSelected(e.target.id)
        }
       
        return <>
            Search this dummy result ....
            <Row className="w-100 border" id={"ID 1"} onClick={handleClick}>{"ID 1"}</Row>
            <Row className="w-100 border" id={"ID 2"} onClick={handleClick}>{"ID 2"}</Row>
            <Row className="w-100 border" id={"ID 3"} onClick={handleClick}>{"ID 3"}</Row>
        </>
    }
